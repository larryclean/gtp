...
func (self *ScoreUtil) Score(mode int, color int32) ScoreResult {
	...
	if mode == E_MODE || (mode == N_MODE && empty <= 4 && self.Kifu.NodeCount >= 150) || self.Kifu.Size < 19 {
		...
		s.CN = fmt.Sprintf("%.2f", (black_territory-white_territory-float64(sear))/2)
		s.JP = fmt.Sprintf("%.2f", (black_territory - white_territory - float64(sear)))
